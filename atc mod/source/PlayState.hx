...
class PlayState extends MusicBeatState
{
	...
	var healingbar:Bool = false;
	private var bublebg:FlxSprite;
	...
	override public function create()
	{
		...
		switch(SONG.stage)
		{
			...
			case 'buble':
			{
				defaultCamZoom = 0.9;
				curStage = 'buble';
				var bg:FlxSprite = new FlxSprite (240, 300).loadGraphic(Paths.image('tazon/fondo'));
                 bg.setGraphicSize(Std.int(FlxG.width * 1.8));
				 bg.screenCenter(X);
				 bg.screenCenter(Y);
				 add(bg);

				 var floor: FlxSprite = new FlxSprite (200, 50).loadGraphic(Paths.image('tazon/suelo'));
				 floor.setGraphicSize(Std.int(FlxG.width * 2.0));
				 floor.screenCenter(X);
				 floor.screenCenter(Y);
				 add(floor);
		...
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Hard" : "Hard") + (Main.watermarks ? " - KE " + MainMenuState.kadeEngineVer : ""), 16);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (healingbar) {
			...
			bublebg = new FlxSprite(1006, 176).loadGraphic(Paths.image('barempty'));
			bublebg.scrollFactor.set();//shadow: bg so it doesnt look like its appearing out of nowhere
			add(bublebg);
	
			redbar = new FlxBar(1006, 176, BOTTOM_TO_TOP, Std.int(bublebg.width), Std.int(bublebg.height), this,
	...
	function addHope(attack:Float) {
		if (canfill)
			FlxTween.tween(this, {attackLevel: attackLevel + attack}, 0.1);

		if (attackLevel > 60) {
			//camHUD.flash(FlxColor.BLUE, 1);
			if (storyDifficulty == 1)
				{
					FlxTween.num(health, health + 0.53, 0.4, {ease: FlxEase.cubeOut}, function (v:Float) {
						health = v;
					});
				}
				else
		...
		for (i in 0...4)
		{
			...
			switch (style)
			{
				...
				case 'normal':
					...
					case 'guitar':
					babyArrow.frames = Paths.getSparrowAtlas('mechanics/notes/NOTE_assets_Calamardo');
					babyArrow.animation.addByPrefix('green', 'arrowUP');
					babyArrow.animation.addByPrefix('blue', 'arrowDOWN');
					babyArrow.animation.addByPrefix('purple', 'arrowLEFT');
					babyArrow.animation.addByPrefix('red', 'arrowRIGHT');
				...
				default:
					babyArrow.frames = Paths.getSparrowAtlas('mechanics/notes/NOTE_assets_Pulpo');
					babyArrow.animation.addByPrefix('green', 'arrowUP');
					babyArrow.animation.addByPrefix('blue', 'arrowDOWN');
					babyArrow.animation.addByPrefix('purple', 'arrowLEFT');
					babyArrow.animation.addByPrefix('red', 'arrowRIGHT');
	...
	public var noesPatricio:Bool = false;
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						if(dad.curCharacter == "pico" && health > 0.15){
							health -= 0.03;
						}
						if(dad.curCharacter == "doodle" && health > 0.15){
							health -= 0.03;
						}
						if(dad.curCharacter == "calamardo" && health > 0.25){
							health -= 0.02;
						}
						if(dad.curCharacter == "pulpo" && health > 0.35){
							health -= 0.01;
						}
					...
					if (daNote.isSustainNote){
						...
						daNote.x += daNote.width / 2 + 17;
					if (daNote.style ==  "doodle") daNote.x -=20;
	...
	*/
	...
	override function beatHit()
	{
		...
		if (curBeat % 16 == 15 && SONG.song == 'Tutorial' && dad.curCharacter == 'gf' && curBeat > 16 && curBeat < 48)
			...
			if (curSong.toLowerCase() == '6am' && curBeat == 206)
					{
				changeDadCharacter('calamardo');
			}

			if (curSong.toLowerCase() == '6am' && curBeat == 399)
				{
			changeDadCharacter('doodle');
		}
	...
	function callDie(){
		...
		generateStaticArrows(0, 'doodle', false);
		generateStaticArrows(1, 'doodle', false);
	...
	function fucking() {
		...
		function flash(id:Bool)  
			{
				if(id)
				{
					camGame.flash(FlxColor.BLUE, 0.40);
					
	
					
				}
            }
		function cam(id:Bool)  
				{
					if(id)
					{
						camGame.flash(FlxColor.RED, 0.40);
						
		
						
					}
				}
				function lantern(id:Bool)  
					{
						if(id)
						{
							camGame.flash(FlxColor.GREEN, 0.40);
							
			
							
						}
					}
					function guarian(id:Bool)  
						{
							if(id)
							{
								camGame.flash(FlxColor.PURPLE, 0.40);
