...
class PlayState extends MusicBeatState
{
	...
	public static var DangerZone:Bool = false;
	private var Danger:FlxCamera;
    var redDanger:FlxSprite;
	...
	override public function create()
	{
		...
		Danger = new FlxCamera();
		Danger.bgColor.alpha = 0;

		camHUD.zoom = PlayStateChangeables.zoom;

		FlxCamera.defaultCameras = [camGame];
		FlxG.cameras.add(Danger);
		...
		switch (songLowercase)
		{
			...
			/*case 'desolate':
				dialogue = CoolUtil.coolTextFile(Paths.txt('desolate/dleDialogue'));
			case 'last-stand':
				dialogue = CoolUtil.coolTextFile(Paths.txt('last-stand/lastDialogue'));
			case 'seven-errors':
				dialogue = CoolUtil.coolTextFile(Paths.txt('seven-errors/sevenDialogue'));*/
		...
		switch(stageCheck)
		{
			...
			case 'virus':
			{
				curStage = 'virus';
				defaultCamZoom = 0.72;
				var bg:FlxSprite = new FlxSprite(-1050, -660).loadGraphic(Paths.image('Destroy/Fondo'));
				bg.antialiasing = FlxG.save.data.antialiasing;
				...
				var stageFront:FlxSprite = new FlxSprite(-650, 300).loadGraphic(Paths.image('Destroy/Piso'));
		...
		redDanger = new FlxSprite(-1050, -660).loadGraphic(Paths.image('update/black_vignette'));
		redDanger.antialiasing = FlxG.save.data.antialiasing;
		//bg.setGraphicSize(Std.int(bg.width * 1.2));
		redDanger.active = true;
		redDanger.screenCenter();
		redDanger.visible = true;
		add(redDanger);

		redDanger.cameras = [Danger];
		...
		if (!PlayStateChangeables.flip)
			{
				...
				healthBar.createFilledBar(0xFF460000, 0xFF56f9fb);
			}
			else   
			{
	...
	/*function cokkieCat(?dialogueBox:DialogueBox):Void{
		if (SONG.song.toLowerCase() == 'desolate'){
			FlxG.camera.fade(FlxColor.WHITE, 0.01, true, function()
				{
					add(dialogueBox);
				}, true);
	...
	function defaultDancing(?dialogueBox:DialogueBox):Void{
		...
		function gflaapalmado(?dialogueBox:DialogueBox):Void{
		if (SONG.song.toLowerCase() == 'seven-errors'){
			FlxG.camera.fade(FlxColor.WHITE, 0.01, true, function()
				{
					add(dialogueBox);
				}, true);
	...
	function startCountdown():Void
	{
		...
		if (SONG.song.toLowerCase() == "desolate" || SONG.song.toLowerCase() == "last-stand")
		{
			new FlxTimer().start(0.004, function(cbt:FlxTimer)
			{
				if (ctrTime == 0)
				{
					var cText = "A          S           W          D";
		...
		if (SONG.song.toLowerCase() == "seven-errors")
		{
			new FlxTimer().start(0.004, function(cbt:FlxTimer)
			{
				if (ctrTime == 0)
				{
					var cText:Array<String> = ['A', 'S', 'D', 'S\nP\nA\nC\nE', 'H', 'J', 'K'];
	...
	override public function update(elapsed:Float)
	{
		...
		if (DangerZone)
			{
				redDanger.alpha = 1 - (health / 1.56); // alpha gets to 1 when near 12% health i think :p
			} else {
				redDanger.alpha = 0;
			}
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if (!daNote.modifiedByLua)
						...
						if(dad.curCharacter == "bfbad" && curSong == 'desolate' && health > 0.10){ 
							health -= 0.042;
						}

					    if(dad.curCharacter == "bfbad" && curSong == 'last-stand' && health > 0.09){ 
							health -= 0.035;
						}
						if(dad.curCharacter == "bf-happy-glitch" && curSong == 'seven-errors' && health > 0.15){
							health -= 0.03;
						}
						if (dad.curCharacter == "bf bad" && curSong == 'Antivirus' && health > 0.10)
						{
                          health -= 0.02;
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (PlayState.storyWeek == 0)
					 ...
					 if (PlayState.storyWeek == 0)
						if (accuracy <= 72) {
						 FlxG.switchState(new BadEnding());
					 }
					 if (PlayState.storyWeek == 0)
						if (accuracy == 75) {
						 FlxG.switchState(new TrollEnding());
					 }

/*if (PlayState.storyWeek == 'tu week')
   FlxG.switchState (new Ending()); */
	...
	*/
	...
	override function beatHit()
	{
		...
		if (curBeat == 10  && SONG.song.toLowerCase() == 'seven-errors')
			{
				cokkie =  new FlxTrail(dad, null, 4, 24, 0.6, 0.9);
				add(cokkie);
			}
        if (curBeat == 0 && SONG.song.toLowerCase() == 'desolate')
		{
             gf.alpha == 0;
		}
		if (curBeat == 0 && SONG.song.toLowerCase() == 'last-stand')
			{
				 gf.alpha == 0;
			}
			if (curBeat == 0 && SONG.song.toLowerCase() == 'seven-errors')
				{
					 gf.alpha == 0;
				}
	/*	if (curStep == 320 && curSong == 'Antivirus')

			{
				changeBFCharacter('bf-worried');
			}


			if (curStep == 480 && curSong == 'Antivirus')

				{
					changeBFCharacter('bf-sad');
				}

				if (curStep == 544 && curSong == 'Antivirus')

					{
						changeBFCharacter('bf');
					}

					if (curStep == 703 && curSong == 'Antivirus')
		...
		if (curStep == 703 && curSong == 'Antivirus')
			...
			if (curStep == 831 && curSong == 'Antivirus')
